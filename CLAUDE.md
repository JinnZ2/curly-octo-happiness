...
3. **Systems Diagnostic Suite (SDS)** — exists twice with incompatible APIs: `modules/` (GAE/HND/FDM split across files, networkx-based, `GeometricApplicabilityEngine.analyze(nodes, edges)`) and `diagnostic/systems_diagnostic_suite.py` (self-contained, `GAE(nodes, edges).analyze()`, plus a `TransitionSimulator`). `project/` connects a GAE to a plotly "Shape Board" (`shape_board.py`), but `gae_shape_connecor.py` (note the filename typo) imports names that exist in neither copy.

## Repository conventions and gotchas

- **Runnable vs. sketch files**: `unified_playground_v6.py` (elided `__init__`/methods), `project/extension.py` (methods meant to be pasted into `shape_board.py`'s `ShapeGenerator`), `octahedral_canon.py` (starts with prose — not valid Python), and `project/Organize.md` are design sketches, not working code. Most `.md` files in `plugins/` are code snippets intended to be merged into an agent's `handle_mentor` method, and `plugins/Light_bridge.md` holds an entire plugin that was never saved as a `.py` file.
...
- The canonical Gray-coding helper is the `gray_bits(value, bands)` pattern found in `unified_playground_v6.py` / `plugins/gravitational.py` (highest band whose threshold ≤ value). `plugins/magnetic.py` has a divergent, broken variant.
